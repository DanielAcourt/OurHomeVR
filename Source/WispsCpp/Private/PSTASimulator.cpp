...
{
    // 1. Run the P0 safety floor check.
    PSTAS_Core::run_pndf_check(Task);

    // 2. If a violation is detected AND the EOA is active, trigger the fallback.
    if (Task.bViolationDetected && EOA_Executive_Interface::IsOverrideActive())
    {
        // 3. Log a critical violation message.
        ...
        // 4. Immediately disable the EOA override.

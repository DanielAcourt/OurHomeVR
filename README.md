...
This repository contains the reference implementation of the "Wisp Symmetry Kernel", a C++20 project born from a 12-year study personal study of computing principles of "relative truth". It is not merely a library, but a philosophical and architectural proposal for solving the critical flaw of 'Static Alignment' in contemporary artificial intelligence. It also has application in game engine simulations and with work and time it could act as a bridge within a robot or a gamestate that allows the identification of potential system crashing events to be turn into a known unknown with principles that allow "oneself" to performance a crictial systems evualation. For example if an Ai encountered a close, door or a Ship encounteres an iceburg in x time because of Y speed and Z sensors data. #Sound the alram.  
